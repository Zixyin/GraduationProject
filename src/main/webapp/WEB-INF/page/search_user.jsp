		  ...
		  laypage.render({
			  ...
			  ,jump: function(obj, first){
			    ...
			    //alert(obj.limit); //得到每页显示的条数	        
   			    $.post("userContent",{"pageSize":obj.limit,"pageNumber":obj.curr},function(data){
					var result = "";
					for(var i = 0;i < data.length;++ i){
						result += "<tr>";
						result += "<td id=\"id\" style=\"display:none\">" + data[i].id +"</td>";
						result += "<td>" + data[i].userid +"</td>";
						result += "<td>" + data[i].username+"</td>";
						result += "<td>" + data[i].category+"</td>";
						result += "<td><a href=\"detail_user?id="+data[i].id+"\" class=\"tablelink\" >详情</a>&nbsp;";
	 					result += "<a href=\"delete_user?id="+data[i].id+"\" class=\"tablelink\" onclick=\"return confirm('确认删除？');\">删除</a></td>";				 
						result += "</tr>";
					}
					$("tbody").html(result);
				}); 
